...
function resize() {
    ...
    if (player) {
        player.w = Math.min(100, canvas.width * 0.25);
...
class Player {
    constructor() {
        this.w = Math.min(100, canvas.width * 0.25);
        this.h = this.w * 0.8;
...
class Heart {
    constructor() {
        const baseSize = window.innerWidth < 768 ? 15 : 20;
        this.size = Math.random() * 20 + baseSize;
